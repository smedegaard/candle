...
impl Map2 for MatMul {
    ...
    fn f<T: 'static + WithDType + num_traits::Num + Copy>(
        ...
        // Print dimensions
        println!("KleidiAI MatMul: M={}, K={}, N={}, batch={}", m, k, n, b);
        ...
        match T::DTYPE {
            DType::F32 => {
                ...
                let rhs_packed_size = kleidiai_rs::ukernels::matmul::pack::kai_rhs_pack_kxn_x32p16x1b_x32_x32_neon::get_rhs_packed_size(n, k);
                let mut rhs_packed = vec![0u8; rhs_packed_size];

                // Zero bias for 6x16 variant
                ...
                // Run the micro-kernel for each batch
                let lhs_stride_bytes = k * std::mem::size_of::<f32>();
            ...
            dtype => Err(Error::UnsupportedDTypeForOp(dtype, "kleidi matmul").bt()),
        }
    }
}
